...
from .transformer_offset import TransformerBlock
...
class Backbone(nn.Module):
    def __init__(self, cfg):
        ...
        for i in range(nblocks):
            ...
            self.transformers.append(TransformerBlock(channel, cfg.model.transformer_dim, nneighbor))#channelï¼š32ï¼Œ64ï¼Œ128ï¼Œ256
...
class PointTransformerCls(nn.Module):
    def __init__(self, cfg):
        ...
        npoints, nblocks, nneighbor, n_c, d_points = cfg.num_point, cfg.model.nblocks, cfg.model.nneighbor, cfg.num_class, cfg.input_dim
   #     print('diaocha:',npoints)
        self.fc2 = nn.Sequential(
            nn.Linear(32 * 2 ** nblocks, 256),#####四层： 32*（2^4），因为每次下采样的时候维度都*2.
            nn.ReLU(),
            nn.Linear(256, 64),
            nn.ReLU(),
            nn.Linear(64, n_c)#####16，40
        )
        self.nblocks = nblocks
    
    def forward(self, x):
      #  print('diaocha:',npoints)
        points, _ = self.backbone(x)######b，1024/(4^nblocks)，32*(2^nblocks)
        res = self.fc2(points.mean(1))###按第二个维度求mean之后输入fc2  ####mean:(16,4,256)->(16,256)
        return res####b，40
